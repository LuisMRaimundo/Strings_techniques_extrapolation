...
def _ordinary() -> list[dict]:
    ...
    rows: list[dict] = []
    for dyn, scale in (("pp", 0.85), ("mf", 1.0), ("ff", 1.1)):
        for i, r in enumerate(reg):
            rows.append(
                {
                    "note": r["note"],
                    "midi": r["midi"],
                    "value": 40.0 * scale * (0.985**i),
                    "instrument": "vln",
                    "dynamic": dyn,
                    "technique": "ordinary",
                    "quantity": "EWSD_score_acoustic_balanced",
                    "source_path": f"synthetic_integration_test://{r['note']}:{dyn}",
...
def test_export_retains_unavailable_harmonics(tmp_path: Path) -> None:
    ...
    assert int(keys["n_unavailable"]) == len(un_df)
    # Ordinary techniques remain numeric; calibrated harmonics add more.
    ...
    nat_numeric = nat[nat["estimate_mean"].notna()]
    assert len(nat_numeric) > 0
    assert (
        nat_numeric["extrapolation_method"] == "harmonic_modal_frequency_with_descriptor_priors"
    ).all()
    ...
    art_numeric = art[art["estimate_mean"].notna()]
    assert len(art_numeric) > 0
    assert (art_numeric["extrapolation_method"] == "harmonic_modal_frequency_with_descriptor_priors").all()
    ...
    assert (
        harm_sel["selected_model_id"] == "harmonic_modal_frequency_with_descriptor_priors"
    ).all()
    for _, row in harm_sel.iterrows():
        assert str(row.get("modal_metadata_status") or "") == "complete"
        assert str(row.get("acoustic_calibration_status") or "") == "available"
        assert "calibrated_harmonic_descriptor_model" not in str(
            row.get("missing_model_components") or ""
        )
