#!/usr/bin/env python3
"""Generate docs/TECHNICAL_GUIDE.md with LaTeX math (StackEdit / MathJax compatible)."""
...
OUT = ROOT / "docs" / "TECHNICAL_GUIDE.md"
ASSETS = ROOT / "docs" / "technical_guide_assets"
...
**Registry:** `configs/acoustic_descriptors.yaml` (descriptor backend).  
`ewsd_compatibility` remains incompatible without activated mapping.

### Spectral centroid

**Formula / method:** `spectral_centroid_v1` — $C=\sum f_k W_k/\sum W_k$ with explicit $W_k\in\{\mathrm{magnitude},\mathrm{power}\}$.

**Measurement domain:** must not equate `string_velocity_at_bow` centroids with `radiated_audio` (Schoonderwaldt extract warning).

### Other descriptors

Implemented backend: slope, HNR (spectral-mask), flux, frame spectral variance, LTAS, partials, typed attenuation.  
Unsupported remain schema-only: loudness, attack time, bridge mobility, etc.
...
Helpers exist but **must not** be used as EWSD density operations:
...
**Implementation:** `prediction/operations.py::amplitude_ratio_from_db`, `power_ratio_from_db`  
...
def write_assets() -> None:
    ...
    (ASSETS / "architecture.mmd").write_text(
        """flowchart LR
  IN[Ingestion] --> PI[ProductionInstruction]
  PI --> ONT[Ontology]
  PI --> APP[Applicability]
  PI --> QC[Qualitative constraints]
  PDF[Corpus PDFs] --> ID[Identity] --> SRC[Sources/Extracts] --> ACT[Activation]
  ACT --> PRED[Prediction]
  UA[User assumptions] --> PRED
  APP --> PRED
  PRED --> OUT[Exports/Reports]
""",
        encoding="utf-8",
    )
    (ASSETS / "applicability_flowchart.mmd").write_text(
        """flowchart TD
  A[param + query] --> B{dims present?}
  B -->|no| I[insufficient_metadata]
  B -->|yes| C{match?}
  C -->|no| N[not_applicable]
  C -->|yes| D{transfer w/o equation?}
  D -->|yes| T[applicable_only_by_explicit_transfer]
  D -->|no| E{contradictory?}
  E -->|yes| X[contradictory_metadata]
  E -->|no| M[matched]
""",
        encoding="utf-8",
    )
    (ASSETS / "prediction_sequence.mmd").write_text(
        """sequenceDiagram
  participant U as Caller
  participant P as Pipeline
  participant E as Evidence/Assumptions
  participant O as Ops+Links
  participant Q as MC UQ
  U->>P: request + mode
  P->>E: resolve active params
  alt active
    E->>O: apply_operation
    O->>Q: propagate_metric_only
    Q-->>U: density summary
...
""",
    ...
    (ASSETS / "evidence_provenance.mmd").write_text(
        """flowchart TB
  F[Filename claim] --> V[Internal title/author/year/DOI]
  V --> H[SHA-256]
  H --> S{validation_status}
  S -->|verified_identity| R[Register source]
  S -->|duplicate/reject| X[Do not ingest as claimed]
  R --> EX[Page extracts]
  EX --> G[Activation gate]
  G -->|fail| Q[Qualitative only / inactive]
  G -->|pass| N[Numerical density param]
""",
        encoding="utf-8",
    )
    (ASSETS / "config_dependency.mmd").write_text(
        """flowchart TB
  ONT[technique_ontology.yaml] --> PROD[production]
  SRC[literature_sources.yaml] --> LIT[literature]
  EV[literature_evidence_extracts.yaml] --> LIT
  PAR[literature_parameters.yaml] --> ACT[activation]
  DM[density_metric.yaml] --> PRED[prediction]
  ML[model_links.yaml] --> PRED
  UA[user_assumptions.yaml] --> PRED
  QC[qualitative_acoustic_constraints.yaml] --> ENG[constraints]
""",
        encoding="utf-8",
    )
    (ASSETS / "README.md").write_text(
        """# Technical guide assets

Mermaid sources referenced by `docs/TECHNICAL_GUIDE.md`:
...
Open the guide in StackEdit (MathJax) for LaTeX rendering of `$...$` / `$$...$$`.
""",
        encoding="utf-8",
    )
